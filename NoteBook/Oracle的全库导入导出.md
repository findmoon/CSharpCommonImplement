...
这个原因在于`@SID`位置放置不正确。密码password和`@SID`之前不能有空格，并且要紧挨着。也就是上面所示的写法。系统认证的`/`也要写成`/@SID`。
...
如果导入导出是不一致，推荐的做法是，删除目标数据库，重新创建与源库字符集一样的数据库，而不是直接进行字符集的修改。
...
自己测试，有些能够修改，并且提示“已更改”，但是查看仍然没有变化。
