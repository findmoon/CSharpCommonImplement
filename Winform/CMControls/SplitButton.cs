...
{
    public enum SplitButtonDropWidth
    {
        Default=0,
        EquivalWidth=1
    }
    ...
    public class SplitButton : Button
    {
        ...
        /// </summary>
        //public LeftRightAlignment SplitMenuAlign { get; set; } = LeftRightAlignment.Left;
        [DefaultValue(ToolStripDropDownDirection.Default), CategoryAttribute("外观"), Description("ContextMenuStrip下拉菜单项的位置")]
        public ToolStripDropDownDirection SplitMenuStripAlign { get; set; }

        [DefaultValue(SplitButtonDropWidth.Default), CategoryAttribute("Appearance"), Description("ContextMenuStrip下拉菜单项是否和SplitButton等宽")]
        public SplitButtonDropWidth SplitMenuStripWidth { get; set; }
        ...
        [CategoryAttribute("Behavior"), DefaultValue(null), Description("指定SplitButton实例的下拉菜单项[推荐使用SplitMenuStrip]")]
        ...
        // CategoryAttribute 属性的分类【在特性中】
        // BrowsableAttribute 当前属性是否可见，默认true（属性或事件可以在设计时(窗体设计器中)修改）
        // ReadOnlyAttribute 特性所绑定到的属性是否为只读属性，默认false
        // DefaultValue 用于指定绑定的属性的默认值，即初始化的实例
        // Description 指定事件或属性的说明
        [CategoryAttribute("Behavior"), BrowsableAttribute(true), ReadOnlyAttribute(false), DefaultValue(null), Description("指定SplitButton实例的下拉菜单项[推荐]")]
        ...
        private void ShowContextMenuStrip()
        {
            ...
            if (m_SplitMenu != null)
            {
                //var point = new Point(0, Height);
                //if (SplitMenuAlign == LeftRightAlignment.Right)
                //{
                //    var menuStrip = new ContextMenuStrip();
                //    if (m_SplitMenu.MenuItems!=null && m_SplitMenu.MenuItems.Count>0)
                //    {
                //        menuStrip.Items.AddRange((ToolStripItem[])m_SplitMenu.MenuItems);
                //    }
                //    foreach (MenuItem item in m_SplitMenu.MenuItems)
                //    {
                //        var t = (ToolStripItem)item;
                //    }
                //    point = new Point(this.Width - , Height);
                //}
                m_SplitMenu.Show(this, new Point(0, Height));
            }
            else if (m_SplitMenuStrip != null)
            {
                if (SplitMenuStripWidth == SplitButtonDropWidth.EquivalWidth)
                {
                    m_SplitMenuStrip.AutoSize = false;
                    m_SplitMenuStrip.Width = this.Width-1;
                    for (int i = 0; i < m_SplitMenuStrip.Items.Count; i++)
                    {
                        m_SplitMenuStrip.Items[i].AutoSize = false;
                        m_SplitMenuStrip.Items[i].Width = this.Width-1;
                    }
                }
                else
                {
                    m_SplitMenuStrip.AutoSize = true;
                }
                var point = new Point(this.Width - m_SplitMenuStrip.Width, Height-1);
                switch (SplitMenuStripAlign)
                {
                    case ToolStripDropDownDirection.AboveLeft:
                        point = new Point(0, -m_SplitMenuStrip.Height+1);
                        break;
                    case ToolStripDropDownDirection.AboveRight:
                        point = new Point(this.Width - m_SplitMenuStrip.Width, -m_SplitMenuStrip.Height + 1);
                        break;
                    case ToolStripDropDownDirection.BelowLeft:
                        point = new Point(0, Height-1);
                        break;
                    case ToolStripDropDownDirection.Left:
                        point = new Point(- m_SplitMenuStrip.Width+1, 0);
                        break;
                    case ToolStripDropDownDirection.Right:
                        point = new Point(this.Width-1, 0);
                        break;
                ...
                m_SplitMenuStrip.Show(this, point);
            }
        }
